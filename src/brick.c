...
void		newBrick(GLfloat x, GLfloat y, GLfloat width, GLfloat height)
{
	...
	new->vertex.direction.y = 0;
	new->vertex.type = V4;
	new->vertex.shape.V4f.x = x;
	new->vertex.shape.V4f.y = y;
	new->vertex.shape.V4f.width = width;
	new->vertex.shape.V4f.height = height;
	new->vertex._update = basicBrickUpdate;
